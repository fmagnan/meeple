...
def init_hand(names: list[Name]) -> Hand:
    hand :Hand = Hand(deck)
    for name in names:
        hand.add_card(name)
    return hand
...
def test_gets_full_points_when_candle_has_bell_tower_and_book_of_changes_and_any_wizard():
    hand = init_hand( [Name.BEASTMASTER, Name.BELL_TOWER, Name.CANDLE])
...
def test_does_not_get_candle_bonus_without_a_wizard():
    hand = init_hand( [Name.SWORD_OF_KETH, Name.BELL_TOWER, Name.CANDLE])
    hand.add_card(Name.BOOK_OF_CHANGES, { 'card': Name.CANDLE, 'suit': Suit.WEAPON})
...
def test_cannot_choose_any_card_with_fountain_of_life_and_only_artifacts():
    hand = init_hand( [Name.FOUNTAIN_OF_LIFE, Name.GEM_OF_ORDER, Name.PROTECTION_RUNE, Name.SHIELD_OF_KETH, Name.WORLD_TREE])
...
def test_add_a_last_card_with_necromancer():
    hand = init_hand( [Name.DRAGON, Name.EARTH_ELEMENTAL])
...
def test_can_have_two_times_the_same_card_because_of_the_doppelganger():
    hand = init_hand( [Name.DRAGON, Name.WARLOCK_LORD])
